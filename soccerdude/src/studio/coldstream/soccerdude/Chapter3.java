...
public class Chapter3 extends BaseExample implements IOnMenuItemClickListener {
	// ===========================================================
	// Constants
	// ===========================================================
	private static final boolean DEBUG_MODE = false;
	
	private static final int THIS_CHAPTER = 3;
	...
	private TiledTextureRegion mSpiderTextureRegion;
	...
	private boolean killedSpiderFlag;
	...
	public void onLoadResources() {
		...
		this.mSpiderTextureRegion = TextureRegionFactory.createTiledFromAsset(this.mTexture, this, "spider.png", 0, 0, 5, 2); 
		this.mEngine.getTextureManager().loadTexture(this.mTexture);
	...
	public Scene onLoadScene() {
		...
		try {
			...
			this.mTMXTiledMap = tmxLoader.loadFromAsset(this, "tmx/level3.tmx");
		...
		ball.setPosition(6.0f * 32, 36.0f * 32);
		...
		/* Create the wasp sprite and add it to the scene. */
		final AnimatedSprite spider = new AnimatedSprite(centerX, centerY, this.mSpiderTextureRegion);
		final PhysicsHandler spiderPhysicsHandler = new PhysicsHandler(spider);
		spider.registerUpdateHandler(spiderPhysicsHandler);		
		//spider.setPosition(6.0f * 32, 35.0f * 32);
		...
		final float wx = (mTMXTiledMap.getTileWidth() * 32) ; //32
		final float wy = (mTMXTiledMap.getTileHeight() * 5) ; //5
		spider.setPosition(wx, wy);
		
		final Path path = new Path(5).to(wx, wy).to(wx+128, wy).to(wx+128, wy+1).to(wx, wy).to(wx, wy+1);
		
		final IEntityModifier spiderModifier = new LoopEntityModifier(new PathModifier(4, path, null, new IPathModifierListener() {
			@Override
			public void onWaypointPassed(final PathModifier pPathModifier, final IEntity pEntity, final int pWaypointIndex) {
				switch(pWaypointIndex) {
					case 0:
						spider.animate(new long[]{150, 150, 150, 150, 150}, 0, 4, true);
						break;
					case 1:
						...
						break;
					case 2:
						spider.animate(new long[]{150, 150, 150, 150, 150}, 5, 9, true);
						break;
					case 3:
						spider.stopAnimation();
		...
		/*final Path path1 = new Path(2).to(wx, wy).to(wx+128, wy);
		final Path path2 = new Path(2).to(wx+128, wy).to(wx, wy);
		
		final IEntityModifier spiderModifier = new LoopEntityModifier(new ParallelEntityModifier(
				new PathModifier(2, path1, null, new IPathModifierListener() {
				@Override
				public void onWaypointPassed(final PathModifier pPathModifier, final IEntity pEntity, final int pWaypointIndex) {
					switch(pWaypointIndex) {
						case 0:
				...
				new PathModifier(2, path2, null, new IPathModifierListener() {
					@Override
					public void onWaypointPassed(final PathModifier pPathModifier, final IEntity pEntity, final int pWaypointIndex) {
						switch(pWaypointIndex) {
		...
		spider.registerEntityModifier(spiderModifier);
		...
		this.mMainScene.registerUpdateHandler(new IUpdateHandler() {
			...
			public void onUpdate(final float pSecondsElapsed) {
				...
				if(wasp.getX() > 0 && wasp.getY() > 0){
					...
					final float cx = (mTMXTiledMap.getTileWidth() * 8) + 75.0f * (float)Math.sin(Chapter3.this.mEngine.getSecondsElapsedTotal() / 2.25);
					final float cy = (mTMXTiledMap.getTileHeight() * 17) + 15.0f * (float)Math.sin(4 * Chapter3.this.mEngine.getSecondsElapsedTotal());
				...
				if(isProp(ballTile,"wall","true") || isProp(ballTile,"sign","true")) {		
					//Bouncing routine					
					//Chapter3.this.mBounceSound.play();
				...
				if(isProp(ballTile,"crate","true") && mBallAcc > 100 && !playerNearBall) {		
					...
					ballTile.setGlobalTileID(mTMXTiledMap, 40); //Pure awesome!!! It does change the tile!!!
					incXP(5, ballTile.getTileX(), ballTile.getTileY(), true);
					if(ballTile.getTileColumn() == 20 && ballTile.getTileRow() == 21){
						keyFlag = true;
						storyText.setText("You found a key!");
				...
				/* PLAYER HIT SIGNS */
				if(isProp(playerTile,"sign","true") && playerTile.getTileColumn() == 7 && playerTile.getTileRow() == 8 && !signFlag1) {					
					signFlag1 = true;
					storyText.setText("The sign reads -\n\n'Dead End'");
					mMainScene.setChildScene(mStoryScene, false, true, true);
				}
				if(playerTile.getTileColumn() != 7 && playerTile.getTileColumn() != 8)				
					signFlag1 = false;
				...
				/* PLAYER HIT DOOR */
				if(isProp(playerTile,"door","true") && keyFlag && !doorFlag) {					
					playerTile.setGlobalTileID(mTMXTiledMap, 40);
					doorFlag = true;
					storyText.setText("Door Unlocked!");
					mMainScene.setChildScene(mStoryScene, false, true, true);
				}
				...
				/* PLAYER HIT SPIDER */
				if(player.collidesWith(spider) && !recoverFlag) {							
					recoverFlag = true;
					scoreBoard.setLife(scoreBoard.getLife() - 2); //Test
					...
					if(mValueX > 0.0f && mValueY > 0.0f){
						...
						final float vx = spider.getX() - player.getX();
						final float vy = spider.getY() - player.getY();
						mImpVectX = vx / ((float) Math.sqrt(vx * vx + vy * vy));
						mImpVectY = vy / ((float) Math.sqrt(vx * vx + vy * vy));
					}							
				...
				/* BALL HIT SPIDER */
				if(ball.collidesWith(spider) && mBallAcc > 100 && !playerNearBall) {	
					spider.unregisterEntityModifier(spiderModifier);
					spider.setPosition(-100, -100);
					mBallAcc = 0;
					incXP(75, ballTile.getTileX(), ballTile.getTileY(), true);
					killedSpiderFlag = true;					
				}
				else if(ball.collidesWith(spider)){
					mImpVectX = -mImpVectX;
					mImpVectY = -mImpVectX;
				}				
				...
				/* Next Level */
				if(playerTile.getTileColumn() == 37 && playerTile.getTileRow() == 2 && !nextChapter){
					nextChapter = true;
					endTimer = Chapter3.this.mEngine.getSecondsElapsedTotal();
					
				}
				
				if(nextChapter && (endTimer + 1.0f) < Chapter3.this.mEngine.getSecondsElapsedTotal()){
					//Plus some time...
					//Next level!					
					nextChapter = false;
					
					Intent mainIntent = new Intent(Chapter3.this,ChapterHandler.class);
					...
					Chapter3.this.startActivity(mainIntent);
					Chapter3.this.finish();
				}
		...
		this.mMainScene.getLastChild().attachChild(spider);	
		//Match timer?
		/*mMainScene.registerUpdateHandler(new TimerHandler(1 / 2.0f, true, new ITimerCallback() {
			@Override
			public void onTimePassed(final TimerHandler pTimerHandler) {
				//elapsedText.setText("Seconds elapsed: " + Chapter3.this.mEngine.getSecondsElapsedTotal());
				//Wasp routine
	...
	public boolean onMenuItemClicked(final MenuScene pMenuScene, final IMenuItem pMenuItem, final float pMenuItemLocalX, final float pMenuItemLocalY) {
		switch(pMenuItem.getID()) {
			case MENU_RESET:
				...
				/* Remove the menu and reset it. */
				this.mMainScene.clearChildScene();		
				Intent mainIntent = new Intent(Chapter3.this,Chapter3.class);
				Chapter3.this.startActivity(mainIntent);
				Chapter3.this.finish();
				//this.mMainScene.setChildScene(analogOnScreenControl);
				return true;
			...
			case MENU_QUIT:
				...
				//this.finish();
				this.mMainScene.clearChildScene();		
				Intent mainIntent2 = new Intent(Chapter3.this,MainMenu.class);
				Chapter3.this.startActivity(mainIntent2);
				Chapter3.this.finish();
				return true;
			case MENU_OK:
				this.mMainScene.clearChildScene();		
	...
	protected MenuScene createStoryScene() {
		...
		final Sprite storyPanel = new Sprite(72,40, Chapter3.this.mStoryPanelRegion);
	...
	public void incXP(int ixp, float ix, float iy, boolean makeDust){
		xp += ixp;
		final ChangeableText pointsText = new ChangeableText(ix, iy, Chapter3.this.mXPFont, "+" + Integer.toString(ixp));
		...
		if(makeDust)
			Chapter3.this.mMainScene.getLastChild().attachChild(dust);
		Chapter3.this.mMainScene.getLastChild().attachChild(pointsText);
	}
	
	public void showGoal(){		
		final ChangeableText pointsText = new ChangeableText(124, 120, Chapter3.this.mMenuFont, "GOAL!");
		pointsText.setBlendFunction(GL10.GL_SRC_ALPHA, GL10.GL_ONE_MINUS_SRC_ALPHA);
		...
		Chapter3.this.scoreBoard.getFirstChild().attachChild(pointsText);
	}
