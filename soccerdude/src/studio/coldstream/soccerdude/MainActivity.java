...
public class MainActivity extends BaseExample implements IOnMenuItemClickListener {
	// ===========================================================
	// Constants
	// ===========================================================
	private static final boolean DEBUG_MODE = false;
	
	private static final int THIS_CHAPTER = 1;
	...
	public Scene onLoadScene() {
		...
		try {
			...
			this.mTMXTiledMap = tmxLoader.loadFromAsset(this, "tmx/level1.tmx");
		...
		wasp.setPosition(900.0f, 800.0f);
		...
		final float wx = (mTMXTiledMap.getTileWidth() * 5) ; 
		final float wy = (mTMXTiledMap.getTileHeight() * 35) ;
		...
		this.mMainScene.registerUpdateHandler(new IUpdateHandler() {
			...
			public void onUpdate(final float pSecondsElapsed) {
				...
				if(wasp.getX() > 0 && wasp.getY() > 0){
					...
					final float cx = (mTMXTiledMap.getTileWidth() * 31) + 80.0f * (float)Math.sin(MainActivity.this.mEngine.getSecondsElapsedTotal() / 2.25);
					final float cy = (mTMXTiledMap.getTileHeight() * 31) + 15.0f * (float)Math.sin(4 * MainActivity.this.mEngine.getSecondsElapsedTotal());
				...
				if(isProp(ballTile,"wall","true") || isProp(ballTile,"sign","true")) {		
					//Bouncing routine					
					//MainActivity.this.mBounceSound.play();
				...
				if(isProp(ballTile,"goal","true") && !goalFlag && killedWaspFlag) {		
					MainActivity.this.mGoalSound.play();
					//GOAL!!!
					//mBallAcc = 0;
					//ball.stopAnimation();
				...
				/* PLAYER HIT SIGNS */
				if(isProp(playerTile,"sign","true") && playerTile.getTileColumn() == 29 && playerTile.getTileRow() == 12 && !signFlag1) {					
					signFlag1 = true;
					storyText.setText("The sign reads -\n\n'Pathway to Kimberly Street'");
					mMainScene.setChildScene(mStoryScene, false, true, true);
				}
				if(playerTile.getTileColumn() != 29 || playerTile.getTileRow() != 12)				
					signFlag1 = false;
				
				if(isProp(playerTile,"sign","true") && playerTile.getTileColumn() == 17 && playerTile.getTileRow() == 32 && !signFlag2) {					
					signFlag2 = true;
					storyText.setText("The sign reads -\n\n'Lord Willows Courtyard'");
					mMainScene.setChildScene(mStoryScene, false, true, true);
				}
				if(playerTile.getTileColumn() != 17 || playerTile.getTileRow() != 32)				
					signFlag2 = false;
				
				/* PLAYER HIT WILLOW */
				if(player.collidesWith(willow) && !willowFlag) {
					willowFlag = true;
					storyText.setText("Please help me!," +
							"\nThere is a big wasp in" +
							"\nour courtyard! If you can" +
							"\nget rid of it I will tell" +
							"\nyou a little secret...'");
				...
				if(shootBall && playerNearBall){
					MainActivity.this.mKickSound.play();
					mImpVectX = mValueX / (float) Math.sqrt(mValueX * mValueX + mValueY * mValueY);
					mImpVectY = mValueY / (float) Math.sqrt(mValueX * mValueX + mValueY * mValueY);
					mBallAcc = 200;
				...
				/* Next Level */
				if(killedWaspFlag && goalFlag && !nextChapter){
					nextChapter = true;
					endTimer = MainActivity.this.mEngine.getSecondsElapsedTotal();
					
				}
				
				if(nextChapter && (endTimer + 5.0f) < MainActivity.this.mEngine.getSecondsElapsedTotal()){
					//Plus some time...
					//Next level!
					killedWaspFlag = false;
					nextChapter = false;
					
					Intent mainIntent = new Intent(MainActivity.this,ChapterHandler.class);
	...
	protected MenuScene createStoryScene() {
		...
		final Sprite storyPanel = new Sprite(72,40, MainActivity.this.mStoryPanelRegion);
	...
	public void incXP(int ixp, float ix, float iy, boolean makeDust){
		xp += ixp;
		final ChangeableText pointsText = new ChangeableText(ix, iy, MainActivity.this.mXPFont, "+" + Integer.toString(ixp));
		...
		if(makeDust)
			MainActivity.this.mMainScene.getLastChild().attachChild(dust);
		MainActivity.this.mMainScene.getLastChild().attachChild(pointsText);
	}
	
	public void showGoal(){		
		final ChangeableText pointsText = new ChangeableText(124, 120, MainActivity.this.mMenuFont, "GOAL!");
		pointsText.setBlendFunction(GL10.GL_SRC_ALPHA, GL10.GL_ONE_MINUS_SRC_ALPHA);
		...
		MainActivity.this.scoreBoard.getFirstChild().attachChild(pointsText);
	}
